...
class Tile
{
	...
	public static function getDeck(bool $starting = false) {
		$deskSign = $starting ? '=' : '<>';
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE deck $deskSign 'starting' AND location = 'deck'" );
		shuffle($deck);
		return $deck;
	}
	
	public static function getBoard() {
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE location = 'board'" );
		return $deck;
	}
	
	public static function getHand($player_id) {
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE location = 'hand' AND player_id = $player_id" );
		return $deck;
	}
	
	public static function getSold($player_id) {
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE location = 'sold' AND player_id = $player_id" );
		return $deck;
	}
	
	public static function getCorruption($player_id) {
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE location = 'corruption' AND player_id = $player_id" );
		return $deck;
	}
	...
	public static function getPlayedCharacters() {
		$deck = SobekTwoPlayers::getObjectList( "SELECT * FROM tile WHERE location = 'played' AND ability <> 12" );
		return $deck;
	}
	...
	public static function giveToPlayer(&$tile, $player_id, $corruption = false) {
		...
		SobekTwoPlayers::DbQuery( "UPDATE tile SET location = '$location', player_id = $player_id WHERE tile_id = $tile[tile_id]" );
	}
	
	public static function discard(&$tile) {
		$tile['location'] = 'discard';
		SobekTwoPlayers::DbQuery( "UPDATE tile SET location = 'discard' WHERE tile_id = $tile[tile_id]" );
	}
	
	public static function discardPlayedCharacter(&$tile) {
		$tile['location'] = 'discard';
		SobekTwoPlayers::DbQuery( "UPDATE tile SET location = 'played' WHERE tile_id = $tile[tile_id]" );
	}
}